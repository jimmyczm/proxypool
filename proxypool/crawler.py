...
class Crawler(object, metaclass=ProxyMetaclass):
    ...
    def gatherproxy(self):
        ...
        html = get_page(start_url, proxies=proxies)
        if html:
            re_ip = re.compile('(\d+\.\d+\.\d+\.\d+)', re.S)
            re_port = re.compile('PROXY_PORT\":\"(.*?)\"\,\"PROXY_REFS', re.S)
            ips = re_ip.findall(html)
            ports = re_port.findall(html)

            for ip, port in zip(ips, ports):
                address_port = ip + ':' + str(int(port, 16))
                yield address_port
